...
class FireSpreadDataset(Dataset):
    def __init__(self, data_dir: str, included_fire_years: List[int], n_leading_observations: int,
        ...
        total_fires = sum([len(self.imgs_per_fire[year]) for year in self.imgs_per_fire])
        total_images = sum([sum([len(imgs) for imgs in fires.values()]) for year, fires in self.imgs_per_fire.items()])
        print(f"Dataset initialized: {total_fires} fires, {total_images} images, {self.length} data points")
        for year in sorted(self.imgs_per_fire.keys()):
            n_fires = len(self.imgs_per_fire[year])
            n_imgs = sum([len(imgs) for imgs in self.imgs_per_fire[year].values()])
            n_datapoints = sum(self.datapoints_per_fire[year].values())
            print(f"  Year {year}: {n_fires} fires, {n_imgs} images, {n_datapoints} data points")
    ...
    def read_list_of_images(self):
        ...
        for fire_year in self.included_fire_years:
            ...
            year_dir = data_dir_path / str(fire_year)

            if not self.load_from_hdf5:
                # Use Path.glob for cross-platform compatibility
                if not year_dir.exists():
                    warnings.warn(f"Year directory not found: {year_dir}", RuntimeWarning)
                    continue
                
                # Find all fire directories (subdirectories of the year directory)
                fires_in_year = [d for d in year_dir.iterdir() if d.is_dir()]
                fires_in_year.sort()
                
                for fire_dir_path in fires_in_year:
                    fire_name = fire_dir_path.name
                    # Find all .tif files in the fire directory
                    fire_img_paths = sorted(fire_dir_path.glob("*.tif"))
                    # Convert Path objects to strings for compatibility
                    fire_img_paths = [str(p) for p in fire_img_paths]
                ...
                # Find all .hdf5 files in the year directory
                hdf5_files = sorted(year_dir.glob("*.hdf5"))
                for fire_hdf5 in hdf5_files:
    ...
    def augment(self, x, y):
        ...
        # If image is smaller than crop size, pad it to crop size first
        if H < self.crop_side_length or W < self.crop_side_length:
            # Pad to at least crop_side_length
            pad_h = max(0, self.crop_side_length - H)
            pad_w = max(0, self.crop_side_length - W)
            # Pad evenly on both sides, with extra padding on right/bottom if needed
            pad_top = pad_h // 2
            pad_bottom = pad_h - pad_top
            pad_left = pad_w // 2
            pad_right = pad_w - pad_left
            x = TF.pad(x, (pad_left, pad_top, pad_right, pad_bottom), padding_mode='reflect')
            y = TF.pad(y, (pad_left, pad_top, pad_right, pad_bottom), padding_mode='reflect')
            H, W = x.shape[-2], x.shape[-1]
        
        # Now we can safely do random crop (image is at least crop_side_length)
        if H >= self.crop_side_length and W >= self.crop_side_length:
            ...
            # Fallback: should never happen after padding, but center crop just in case
            x = TF.center_crop(x, (self.crop_side_length, self.crop_side_length))
            y = TF.center_crop(y, (self.crop_side_length, self.crop_side_length))
